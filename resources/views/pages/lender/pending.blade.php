@php
    ...
    $pending = Application::join('users', 'users.email', 'applications.email')->join('t_post', 'post_id', 'product_id')->where('status', 0)->get();
@endphp
<x-app-layout>
                            ...
                            <tbody>
                                @foreach ($pending as $index => $data)
                                    ...
                                    <tr onclick="go_to('/applicant/apply/{{ $data->id }}')">
                                        ...
                                        <td>₱ {{ number_format(($data->post_install - $data->downpayment) / $data->terms , 2) }} / Month</td>
                                        <td>
                                            <span
